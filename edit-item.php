...
function updateItem($id, $item_name, $item_price, $quantity, $section_id) {
    $conn = connection();
    $sql = "UPDATE items
    ...
    if ($conn->query($sql)) {
        ...
        die("Error updating the item: " . $conn->error);
    }
}
...
if (isset($_POST['btn_add'])) {
    ...
    updateItem($id, $item_name, $item_price, $quantity, $section_id);
}
?>
    ...
    <main class="container">
        <div class="card mx-auto" style="width: 25rem;">
            <div class="card-header bg-info text-white text-center">
                <h2 class="fw-light">Edit Item</h2>
            </div>
            <div class="card-body">
                <form action="" method="POST">
                    <div class="mb-3">
                        <lable class="form-lable">Item Name</lable>
                        <input type="text" name="name" id="name" class="form-control mt-2" maxlength="50" value="<?= $item['item_name'] ?>" required autofocus>
                    </div>
                    <div class="mb-3">
                        <label for="price" class="form-label fw-bodl">Item Price</label>
                        <div class="input-group">
                            <div class="input-group-text">$</div>
                            <input type="number" name="price" id="price" class="form-control" value="<?= $item['item_price'] ?>" min="0" required>
                        </div>
                    </div>
                    <div class="mb-3">
                    ...
                    <button type="submit" name="btn_add" class="btn btn-info fw-bold px-5 text-white">
                        <i class="fa-solid fa-check text-white"></i> Update
                    </button>
                </form>
            </div>
